...
OUIDtemp={};
for i=1:length(OUID)
    OUIDtemp{i}=char(OUID{1,i});
end
for i=1:length(bestneighbours)
    index=strfind(OUIDtemp,bestneighbours{i});
    x=find(not(cellfun('isempty',index)));
    if x>0
        prate(k,:)=datas20(x,:);
        k=k+1;
...
tau=countrates;
rho=length(highly)+length(highly_not)+1.2*length(recomms)+length(recomm_not);
coverage=tau/rho;
    ...
    for j=1:size(prerates,2)
        if ~isnan(prerates(i,j))
            calc=calc+(prerates(i,j)-prate(i,j))^2;
             if prerates(i,j)~=0 || ~isnan(prerates(i,j))
                count=count+1;
            end
        end
    end
